...
RULES = [
    ...
    # ---- Accounting depth (Chartered-Accountant patterns) -------------------
    dict(id="bank_rec", strong=["bank reconciliation", "bank statement", "reconcile bank",
         "statement import", "camt", "match payments"],
         weak=["reconcile", "statement", "bank feed"],
         modules=_m("account"), verdict="configurable", effort="Medium",
         desc="Bank statement import (CAMT/OFX/CSV) with reconciliation models/rules is standard.",
         path="Configure bank journals, statement import and reconciliation rules."),
    dict(id="fixed_assets", strong=["fixed asset", "depreciation", "asset register",
         "asset disposal", "capitalise"],
         weak=["asset", "depreciat", "amortis", "wdv"],
         modules=_m("account"), verdict="configurable", effort="Medium",
         desc="Asset models with automated depreciation boards and disposal are standard (account_asset).",
         path="Configure asset models, depreciation methods and disposal — no code."),
    dict(id="analytic", strong=["cost center", "cost centre", "analytic account",
         "department p&l", "project profitability", "branch p&l"],
         weak=["analytic", "cost centre", "segment", "dimension"],
         modules=_m("account"), verdict="configurable", effort="Medium",
         desc="Analytic accounting (plans + distribution) delivers cost centres and segment P&L.",
         path="Configure analytic plans and default distributions; tag documents."),
    dict(id="budget", strong=["budget", "budgetary control", "budget vs actual"],
         weak=["budget", "forecast spend"],
         modules=_m("account"), verdict="configurable", effort="Medium",
         desc="Budgetary positions track budget vs analytic actuals.",
         path="Configure budgetary positions and budgets against analytic accounts."),
    dict(id="multicurrency", strong=["multi-currency", "multi currency", "foreign currency",
         "exchange rate", "fx revaluation", "unrealised gain"],
         weak=["currency", "fx", "forex", "exchange"],
         modules=_m("account"), verdict="configurable", effort="Medium",
         desc="Multi-currency with period-end revaluation and rate providers is standard.",
         path="Enable currencies, a rate provider and the FX revaluation routine."),
    dict(id="revenue_recognition", strong=["revenue recognition", "deferred revenue",
         "deferred income", "unbilled revenue", "ifrs 15", "milestone billing"],
         weak=["deferred", "accrued revenue", "recognition"],
         modules=_m("account"), verdict="configurable", effort="Medium",
         desc="Deferred revenue/expense models and milestone invoicing handle IFRS 15 recognition.",
         path="Configure deferred revenue models and milestone/percentage invoicing."),
    dict(id="wht", strong=["withholding tax", "wht", "tax deducted at source", "tds"],
         weak=["withholding", "deduction at source"],
         modules=_m("account"), verdict="configurable", effort="Medium",
         desc="Withholding tax codes/groups and reporting are configurable.",
         path="Set up withholding tax codes and the statutory report per regime."),
    dict(id="landed_cost", strong=["landed cost", "import cost", "freight allocation",
         "customs duty allocation"],
         weak=["landed", "freight", "customs", "duty"],
         modules=_m("stock"), verdict="configurable", effort="Medium",
         desc="Landed costs allocate freight/duty/insurance onto inventory value (stock_landed_costs).",
         path="Enable landed costs and configure split methods."),
    dict(id="dropship", strong=["drop ship", "dropship", "dropshipping", "back-to-back order"],
         weak=["dropship", "direct delivery from vendor"],
         modules=_m("stock", "purchase"), verdict="configurable", effort="Low",
         desc="Dropshipping route delivers directly from vendor to customer.",
         path="Enable the dropship route on products; no code."),
    dict(id="quality", strong=["quality control", "quality check", "qc inspection",
         "quality alert"],
         weak=["quality", "inspection", "qc"],
         modules=_m("stock", "mrp"), verdict="standard", effort="Low",
         desc="The Quality app adds control points and checks to operations (enterprise).",
         path="Configure quality control points on receipts/manufacturing."),
    dict(id="commission", strong=["sales commission", "commission plan", "incentive scheme",
         "salesperson commission"],
         weak=["commission", "incentive", "bonus scheme"],
         modules=_m("sale_management"), verdict="custom", effort="High",
         desc="Odoo has no full standard commission engine; tiered/complex schemes need a module.",
         path="Build a commission module: plans, rules per salesperson/product, accrual and payout.",
         design="Models for commission plan/rule/line; compute on confirmed/paid orders; analytic posting; report and payout export."),
]
