...
const readGroup  = (conn, ...a) => odoo.readGroup(conn, ...a);
...
async function getBankBalances(conn) {
...
async function getAgedReceivables(args, conn) {
...
async function getAgedPayables(args, conn) {
  const asOf = args.as_of_date || today();
...
async function getFffPosition(conn) {
  const bills = await searchRead(
    conn, 'account.move',
    [
      ['move_type',     '=',  'in_invoice'],
...
async function getPlSummary(args, conn) {
...
async function getCurrencyRates(conn) {
...
async function executeTool(name, input, conn) {
  switch (name) {
    case 'get_bank_balances':    return getBankBalances(conn);
    case 'get_aged_receivables': return getAgedReceivables(input, conn);
    case 'get_aged_payables':    return getAgedPayables(input, conn);
    case 'get_fff_position':     return getFffPosition(conn);
    case 'get_pl_summary':       return getPlSummary(input, conn);
    case 'get_currency_rates':   return getCurrencyRates(conn);
